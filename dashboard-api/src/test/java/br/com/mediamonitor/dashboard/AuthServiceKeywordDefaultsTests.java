...
class AuthServiceKeywordDefaultsTests {
    ...
    @Test
    void newAccountsReceiveBothMcsKeywords() {
        ...
        verify(jdbc).update(INSERT_USER_KEYWORD, 7L, "MCS");
    }

    @Test
    void startupAddsOnlyTheNewMcsDefaultsToExistingAccountsOnce() {
        when(jdbc.queryForObject(
        ...
        verify(jdbc).update(
                contains("SELECT id, ? FROM dashboard_users"),
                eq("Movimento Cultural Social")
        );
        verify(jdbc).update(
                contains("SELECT id, ? FROM dashboard_users"),
                eq("MCS")
        ...
        verify(jdbc).update(
                "INSERT IGNORE INTO system_migrations(migration_key) VALUES ('default_keywords_mcs_v1')"
        );
    }
}
