...
public class DashboardService {
    ...
    public Map<String, Object> filters() {
        ...
        result.put("keywords", jdbc.queryForList("""
                ...
                JSON_TABLE(c.matched_keywords, '$[*]' COLUMNS(keyword VARCHAR(255) PATH '$')) jt
                ORDER BY jt.keyword
                """, String.class));
        return result;
    }
    ...
    private ArticleRow row(ResultSet rs) throws java.sql.SQLException {
        return new ArticleRow(
                rs.getLong("id"), rs.getString("title"), rs.getString("url"), rs.getString("source"),
    ...
    private List<Map<String, Object>> keywordCount(List<ArticleRow> rows) {
        Map<String, Long> counts = rows.stream().flatMap(r -> r.keywords.stream()).collect(Collectors.groupingBy(k -> k, Collectors.counting()));
    ...
    private record ArticleRow(long id, String title, String url, String source, String section, String journalist,
